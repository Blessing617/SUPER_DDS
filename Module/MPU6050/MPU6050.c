...
{
 short dat[3];
 if (mpu_get_gyro_reg(dat, 0) == 0)
 {
 gx = dat[0];
 gy = dat[1];
 gz = dat[2];
 }
 if (mpu_get_accel_reg(dat, 0) == 0)
 {
 ax = dat[0];
 ay = dat[1];
 az = dat[2];
...
float Angle[2];	
void calculate(short ax,short ay,short az)
{
  Angle[0] = atan2(ax,az)*180/3.14f;
  Angle[1] = atan2(ay,az)*180/3.14f;
}
